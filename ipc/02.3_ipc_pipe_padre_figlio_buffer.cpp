...
/**
 * @brief Semplice funzione per la stampa di un array di numeri interi
 *
 * @param array Array di cui è richiesta la stampa a monitor.
 * @param len Lunghezza dell'array che si desidera stampare.
 */
...
int main()
{
    ...
    else if (leia == 0) // Processo figlio
    {
         ...
         */
        SPLIT(100, '-');
        cout << "Leia: invio sulla pipe l'array: ";
        printArray(sendInt, aLen(sendInt));
        bytes = write(r2d2[1], sendInt, sizeof(sendInt));
    ...
    else // Processo padre
    {
        ...
        /**
         * Lettura dell'array completo in una sola volta.
         *
         * Il processo figlio ha scritto 10 elementi all'interno della pipe.
         * Il processo padre legge gli elementi tutti in una volta.
         */
        SPLIT(100, '=');
        cout << "Byte letti: " << read(r2d2[0], recvInt, sizeof(recvInt)) << endl;
        cout << "Obi-Wan: ricezione dall'array: ";
        printArray(recvInt, aLen(recvInt));
